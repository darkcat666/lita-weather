...
module MyMechanize
  ...
  class Weather
    ...
    AREA_URL = {
      ...
      fukuoka: 'http://weather.yahoo.co.jp/weather/jp/40/8210/40133.html'      
    }.freeze
    ...
    # Returns available area keys
    # @return area [Array]
    def self.available_area
      AREA_URL.keys.map(&:to_s)
    end
    ...
    def format
      ...
      <<-EOF.lines.map(&:strip)
      Area: #{w[:area]}
      Weather: #{w[:today_weather]}
      Temperature(℃): High #{w[:highest_temperature]} / Low #{w[:lowest_temperature]}
